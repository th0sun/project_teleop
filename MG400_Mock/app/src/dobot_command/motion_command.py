...
class MotionCommands:
    ...
    def MovJ(self, args):
        ...
        if self.__dobot.get_robot_mode() not in [
                robot_mode.MODE_ENABLE, robot_mode.MODE_RUNNING]:
    ...
    def JointMovJ(self, args):
        """JointMovJ — move to joint target.

        Syntax (per Dobot TCP/IP Interface Guide V1.6.0):
            JointMovJ(J1, J2, J3, J4, SpeedJ=R, AccJ=R, CP=R)

        The robot must be in ENABLE mode to accept a new joint command.
        """
        if self.__dobot.get_robot_mode() not in [robot_mode.MODE_ENABLE, robot_mode.MODE_RUNNING]:
            self.__dobot.log_warning_msg("The robot mode is not enable or running.")
            return False
        ...
        # Parse per-command optional parameters confirmed in official PDF spec:
        #   SpeedJ=R  — override joint speed ratio for this command (1–100)
        #   AccJ=R    — override joint acceleration ratio for this command (1–100)
        #   CP=R      — continuous-path ratio to next waypoint (0–100)
        for arg in args[4:]:
            ...
            try:
                ...
                continue
            if key == "CP":
                self.__dobot.set_cp_ratio(v)
            elif key == "SpeedJ":
                self.__dobot.set_speed_j_rate(v)
            elif key == "AccJ":
                self.__dobot.set_acc_j_rate(v)
