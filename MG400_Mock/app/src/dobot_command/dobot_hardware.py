...
class DobotHardware:
    ...
    def __init__(self) -> None:
        ...
        self.__q_segment_start = np.array([0.0] * 6)  # start of current segment for CP calc
        ...
        self.__tool_vector_blend_thru = np.array([0.0] * 6)  # previous waypoint for CP Bezier
        self.__tool_in_direction = np.zeros(3)  # unit vector: incoming direction of current segment
        self.__time_index = 0
    ...
    def motion_unstack(self):
        ...
        if self.__robot_mode is robot_mode.MODE_JOG:
            return True, None  # Always wait during jog — CP does not apply

        if self.__robot_mode is robot_mode.MODE_RUNNING:
            if self.__cp_ratio == 0:
                # CP=0: wait until current motion finishes (right-angle corner)
                return True, None

            # CP>0: dequeue early to blend corners.
            # Blend radius r (mm) = fraction of the kinematic stopping distance:
            #   r = CP/100 * v² / (2a)
            # At CP=100 the robot starts blending exactly where it would need to
            # begin decelerating to stop — all corners share the same metric radius
            # regardless of segment length, matching real MG400 CP behaviour.
            r_blend = ((self.__cp_ratio / 100.0)
                       * (self.__speed_l ** 2)
                       / (2.0 * max(self.__acc_l, 1e-6)))
            remaining_tcp = float(np.linalg.norm(
                self.__tool_vector_target[:3] - self.__tool_vector_actual[:3]))
            if remaining_tcp > r_blend:
                return True, None
    ...
    def __register_init_status(self):
        ...
        self.__q_segment_start = self.__q_actual.copy()
        self.__q_init = self.__q_actual
        self.__tool_vector_init = forward_kinematics_b2t(
            self.__q_actual, self.__tool_coord[self.__tool_index])
        # Save incoming unit direction for subsequent CP corner blending
        p_curr = self.__tool_vector_init[:3]
        p_tgt  = self.__tool_vector_target[:3]
        d = p_tgt - p_curr
        n = float(np.linalg.norm(d))
        if n > 1e-6:
            self.__tool_in_direction = d / n
    ...
    def set_tool_vector_target(self, tool_vec_target: List[float]):
        ...
        with self.__lock:
            try:
                # Save current target as the blend-through control point for
                # quadratic Bezier CP blending in generate_target_in_tool.
                self.__tool_vector_blend_thru = self.__tool_vector_target.copy()
    ...
    def generate_target_in_joint(self):
        ...
        with self.__lock:
            # Always generate a full trapezoidal trajectory so that __qd_actual
            # is properly populated for CP blend-radius calculation.
            if self.__robot_mode == robot_mode.MODE_RUNNING and self.__cp_ratio > 0:
                q_init = self.__q_actual.copy()
                self.__q_segment_start = self.__q_actual.copy()
            else:
                self.__register_init_status()
                q_init = self.__q_init
            
            is_cp = (self.__robot_mode == robot_mode.MODE_RUNNING
                     and self.__cp_ratio > 0)
            q_trajs = []
            len_max = 0
            for idx, (q_i, q_target) in enumerate(zip(q_init, self.__q_target)):
                if is_cp and abs(q_target - q_i) > 1e-6:
                    direction = np.sign(q_target - q_i)
                    v_s = max(0.0, direction * self.__qd_actual[idx])
                else:
                    v_s = 0.0
                traj = gene_trapezoid_traj(q_i, q_target, self.__acc_j,
                                           self.__speed_j, self.__timestep, v_s=v_s)
    ...
    def generate_target_in_tool(self):
        ...
        with self.__lock:
            if self.__robot_mode == robot_mode.MODE_RUNNING and self.__cp_ratio > 0:
                tool_vector_init = self.__tool_vector_actual.copy()
                blend_thru_xyz: np.ndarray = self.__tool_vector_blend_thru[:3].copy()
                self.__q_segment_start = self.__q_actual.copy()
                use_bezier = True
            else:
                self.__register_init_status()
                tool_vector_init = self.__tool_vector_init
                blend_thru_xyz = np.zeros(3)  # unused when use_bezier=False
                use_bezier = False

            p_start = tool_vector_init[:3]
            p_end   = self.__tool_vector_target[:3]
            r_start = float(tool_vector_init[3])
            r_end   = float(self.__tool_vector_target[3])

            if use_bezier and np.linalg.norm(p_end - p_start) > 1e-6:
                # ── Correct CP corner blending ────────────────────────────────
                # D  = p_start  (current TCP pos = departure point on incoming line)
                # B  = blend_thru_xyz  (the corner waypoint = Bezier control pt)
                # E  = B + r * unit_out  (symmetric entry point on outgoing line)
                # C  = p_end  (next destination)
                #
                # Path: arc(D, B, E)  then  straight E → C
                # This keeps every corner at the same metric blend radius and
                # leaves straight segments intact between blend zones.
                D = p_start
                B = blend_thru_xyz
                C = p_end

                r_actual  = float(np.linalg.norm(B - D))   # ≈ r_blend from motion_unstack
                vec_BC    = C - B
                dist_BC   = float(np.linalg.norm(vec_BC))

                if dist_BC > 1e-6 and r_actual > 1e-3:
                    unit_out = vec_BC / dist_BC
                    # E must not overshoot the midpoint of B→C
                    r_use    = min(r_actual, dist_BC * 0.5)
                    E        = B + r_use * unit_out

                    # Save outgoing direction as incoming direction for the next corner
                    self.__tool_in_direction = unit_out

                    # ── Phase 1: quadratic Bezier arc  D → B(ctrl) → E ───────
                    N_FINE = 200
                    ts     = np.linspace(0.0, 1.0, N_FINE)
                    w0, w1, w2 = (1 - ts)**2, 2*ts*(1-ts), ts**2
                    bez = (np.outer(w0, D) + np.outer(w1, B)
                           + np.outer(w2, E))              # (N_FINE, 3)
                    seg_lens = np.linalg.norm(np.diff(bez, axis=0), axis=1)
                    cum_arc1 = np.concatenate([[0.0], np.cumsum(seg_lens)])
                    total_arc1 = float(cum_arc1[-1])

                    # ── Phase 2: straight E → C ───────────────────────────────
                    dist_EC   = float(np.linalg.norm(C - E))
                    total_arc = total_arc1 + dist_EC

                    v_s = np.linalg.norm(self.__TCP_speed_actual[:3]) if (self.__robot_mode == robot_mode.MODE_RUNNING and self.__cp_ratio > 0) else 0.0
                    arc_profile = gene_trapezoid_traj(
                        0.0, total_arc, self.__acc_l, self.__speed_l, self.__timestep, v_s=v_s)

                    pos_list = []
                    for s in arc_profile:
                        if s <= total_arc1:
                            s_c  = float(np.clip(s, 0.0, total_arc1))
                            idx  = int(np.searchsorted(cum_arc1, s_c, side="right")) - 1
                            idx  = int(np.clip(idx, 0, N_FINE - 2))
                            dnom = seg_lens[idx] if seg_lens[idx] > 1e-12 else 1e-12
                            frac = (s_c - cum_arc1[idx]) / dnom
                            pos_list.append(bez[idx] + frac * (bez[idx + 1] - bez[idx]))
                        else:
                            t_ec = (s - total_arc1) / dist_EC if dist_EC > 1e-6 else 1.0
                            pos_list.append(E + min(t_ec, 1.0) * (C - E))
                    pos_list = np.array(pos_list)

                else:
                    # Corner too close or degenerate — fall back to straight line
                    dist = float(np.linalg.norm(p_end - p_start))
                    v_s = np.linalg.norm(self.__TCP_speed_actual[:3]) if (self.__robot_mode == robot_mode.MODE_RUNNING and self.__cp_ratio > 0) else 0.0
                    arc_profile = gene_trapezoid_traj(
                        0.0, dist, self.__acc_l, self.__speed_l, self.__timestep, v_s=v_s)
                    direction = (p_end - p_start) / dist if dist > 1e-6 else np.zeros(3)
                    pos_list  = np.array([s * direction + p_start for s in arc_profile])
            else:
                # Standard straight-line trapezoidal trajectory
                dist = float(np.linalg.norm(p_end - p_start))
                v_s = np.linalg.norm(self.__TCP_speed_actual[:3]) if (self.__robot_mode == robot_mode.MODE_RUNNING and self.__cp_ratio > 0) else 0.0
                arc_profile = gene_trapezoid_traj(
                    0.0, dist, self.__acc_l, self.__speed_l, self.__timestep, v_s=v_s)
                ...
                pos_list  = np.array([s * direction + p_start for s in arc_profile])
                if dist > 1e-6:
                    self.__tool_in_direction = direction

            r_x_traj = gene_trapezoid_traj(
                r_start, r_end, self.__acc_j, self.__speed_j, self.__timestep)

            n_pos, n_rot = len(pos_list), len(r_x_traj)
            if n_rot < n_pos:
                r_x_traj = np.concatenate(
                    [r_x_traj, [r_x_traj[-1]] * (n_pos - n_rot)])
            elif n_rot > n_pos:
                pos_list = np.concatenate(
                    [pos_list, [pos_list[-1]] * (n_rot - n_pos)])

            self.__tool_vector_target_set = []
            self.__q_target_set = []
            for pos, r_x in zip(pos_list, r_x_traj):
                full_pos = np.concatenate([pos, [r_x, 0.0, 0.0]])
                try:
                    angles = inverse_kinematics_t2b(
                        full_pos, self.__tool_coord[self.__tool_index])
                    self.__tool_vector_target_set.append(full_pos)
    ...
    def generate_jog_target(self, axis_id):
        ...
        with self.__lock:
            ...
            if tool_vec is None or angles is None:
                return False
    ...
    def __q_controller(self):
        """__q_controller — replay pre-generated trapezoidal trajectory."""
        if self.__robot_mode is robot_mode.MODE_RUNNING:
            # Replay the pre-generated trapezoidal trajectory step-by-step.
            # This matches the real MG400 which plans the full trajectory ahead
            # of time and replays it at the feedback rate.
    ...
    def __update_actual_status(self):
        """__update_actual_status — compute FK and velocity from trajectory replay."""
        try:
