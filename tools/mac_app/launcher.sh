#!/usr/bin/env bash
# Native SwiftUI launcher backend for the same ROS2 stack as ./startup.
...
clear_stale_tmux_if_safe() {
  ...
  echo "Closing stale ./startup tmux session '${TMUX_SESSION}' because no ROS container is running."
...
start_stack() {
  ...
  if ! clear_stale_tmux_if_safe; then
    echo "Refusing to replace active ./startup tmux session '${TMUX_SESSION}'. Stop ./startup first, or keep using that terminal session." >&2
    exit 1
  fi
  ...
  if [[ "${PROJECT_TELEOP_START_RVIZ}" == "1" ]]; then
  ...
  else
    {
      ...
      echo "Run with PROJECT_TELEOP_START_RVIZ=1 to enable it, or use ./startup for the tmux RViz pane."
...
stop_stack() {
  ...
  if ! clear_stale_tmux_if_safe; then
    echo "Active ./startup tmux session detected; app stop will not kill it because the ROS container is still running."
    echo "Stop that session from Terminal if you are using ./startup."
    return 0
  fi
