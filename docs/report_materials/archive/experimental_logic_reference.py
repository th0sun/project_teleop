...
"""
Archived experimental control logic modes.

These strategies were used during development to compare command timing,
filtering, feedforward, and CP behavior.

They are retained as research/reference material for the project report, but
they are not the submitted production runtime. The final project uses
the queue-aware TeleopController path.

This is an archived snapshot, not an executable module in the ROS2 package.
...
Historical usage from vr_teleop_node.py:
...
Do not describe these modes as final user-selectable modes in the report.
"""
